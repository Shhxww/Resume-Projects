...
```bash
# 安装向量化脚本依赖
pip install pymysql lancedb pyarrow openai
...
```bash
# 克隆项目
git clone https://github.com/Shhxww/B1ueMusic2.5.git
cd B1ueMusic2.5
...
```bash
# 运行 Fluss 表初始化脚本
java -cp INIT/target/INIT-1.0-SNAPSHOT.jar fluss.fluss_ddl
```

**Fluss 表包括**：
- 维度表：`dim_user`, `dim_song`, `dim_singer`, `dws_user_profile_rt`
- ODS 层：`ods_content_play_log`, `ods_content_collect_log`, `ods_content_comment_log`, `ods_content_follow_log`, `ods_user_login_log`, `ods_dirty`
- DWD 层：`dwd_content_play_rt`, `dwd_content_collect_rt`, `dwd_content_follow_rt`, `dwd_content_comment_rt`

#### Paimon 表初始化

Paimon 离线表 DDL 定义位于 `INIT/src/main/ddl/paimon/paimon_ddl.sql`，包含以下 4 个离线表：

1. **dwd_concet_song_play_di** - 歌曲播放明细表
2. **dwd_trade_album_order_di** - 专辑订单明细表
3. **dwd_trade_cancel_order_di** - 取消订单明细表
4. **dwd_trade_vip_order_di** - VIP订单明细表
...
使用 Spark 或 Flink SQL 执行 DDL：
```bash
# 使用 Spark SQL
spark-sql -f INIT/src/main/ddl/paimon/paimon_ddl.sql

# 或使用 Flink SQL
sql-client.sh -f INIT/src/main/ddl/paimon/paimon_ddl.sql
```
...
```bash
# 播放日志实时入湖
java -cp ODS/target/ODS-1.0-SNAPSHOT.jar fluss.ods_content_play_log

# 收藏日志实时入湖
java -cp ODS/target/ODS-1.0-SNAPSHOT.jar fluss.ods_content_collect_log

# 评论日志实时入湖
java -cp ODS/target/ODS-1.0-SNAPSHOT.jar fluss.ods_content_comment_log

# 关注日志实时入湖
java -cp ODS/target/ODS-1.0-SNAPSHOT.jar fluss.ods_content_follow_log

# 登录日志实时入湖
java -cp ODS/target/ODS-1.0-SNAPSHOT.jar fluss.ods_user_login_log
```
...
```bash
# 播放事实表ETL
java -cp DWD/target/DWD-1.0-SNAPSHOT.jar fluss.dwd_content_play_rt

# 收藏事实表ETL
java -cp DWD/target/DWD-1.0-SNAPSHOT.jar fluss.dwd_content_collect_rt

# 关注事实表ETL
java -cp DWD/target/DWD-1.0-SNAPSHOT.jar fluss.dwd_content_follow_rt

# 登录事实表ETL
java -cp DWD/target/DWD-1.0-SNAPSHOT.jar fluss.dwd_user_login_rt
```
...
```bash
# 用户画像实时计算
java -cp DWS/target/DWS-1.0-SNAPSHOT.jar app.dws_user_profile_rt

# 热门歌曲统计
java -cp DWS/target/DWS-1.0-SNAPSHOT.jar app.dws_traffic_pop_song

# 热门省份统计
java -cp DWS/target/DWS-1.0-SNAPSHOT.jar app.dws_traffic_pop_province

# 歌手关注统计
java -cp DWS/target/DWS-1.0-SNAPSHOT.jar app.dws_traffic_singer_follow
```
...
```bash
# Paimon 维度表 CDC 同步
java -cp DIM/target/DIM-1.0-SNAPSHOT.jar Paimon.dim_app
```
...
```bash
# 使用提供的 k8s.yaml 文件部署
kubectl apply -f DWD/src/main/java/fluss/k8s.yaml
```
