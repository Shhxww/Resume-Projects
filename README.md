...
**🔐 私密项目地址**：https://github.com/Shhxww/B1ueMusic2.5
...
- 🚀 **Fluss 实时存储**：替代传统 Kafka + Redis/HBase 架构，支持列式裁剪和毫秒级查询
...
│       └── sql/sparksql/
│           ├── dwd_content_song_play_di.sql   # 歌曲播放明细表
...
- ✅ JSON 解析容错
- ✅ Fluss 数据湖同步
- ✅ **列式裁剪**：只读取需要的列，减少网络传输和存储成本
- ✅ **数据一致性**：保证 exactly-once 语义，避免数据丢失或重复
- ✅ **毫秒级延迟**：相比传统 Kafka 消息队列，实时性更强
...
**离线表**（Paimon 存储）📝：
- `dwd_content_song_play_di` - 歌曲播放明细表
  - 分区字段：etl_load_date
  - 主键：(user_id, start_ts)
  - 分桶：user_id（2个桶）
  - 存储格式：Parquet
...
**数据来源**：
- PostgreSQL CDC 实时同步（通过 Fluss）
- Fluss 定期自动刷新到 Paimon（120秒周期）

**CDC 同步流程**（PostgreSQL → Fluss → Paimon）：
1. PostgreSQL Debezium CDC 捕获变更
2. Flink 读取 CDC 数据流（`dim_app_fluss.java`）
3. 解析 CDC 数据并转换为 RowData 格式
4. 通过侧输出流路由到不同的 Fluss 维度表（dim_user/dim_song/dim_singer）
5. Fluss 自动定期刷新到 Paimon 数据湖（120秒周期，通过 `table.datalake.freshness` 配置）

**为什么选择 Fluss 而非直接 Paimon？**
- Fluss 提供毫秒级实时查询能力，适合实时维表关联
- Fluss 定期自动刷新到 Paimon（120秒），实现湖仓一体
- Paimon 用于离线分析和批量查询，提供更好的 OLAP 性能
...
**实现方式**：
- 使用 Flink SQL 的 **Lookup Join** 语法
- 支持实时维度变更（维度数据存储在 Fluss 中）
- 维度数据自动刷新到 Paimon（120秒周期）
- 支持远程缓存优化
...
CREATE TABLE dim_user (
    ...
    'connector' = 'fluss',
    'bucket.num' = '2',
    'table.datalake.enabled' = 'true',    -- 启用数据湖功能
    'table.datalake.freshness' = '120s', -- 120秒自动刷新到 Paimon
...
**PostgreSQL CDC → Fluss**（`dim_app_fluss.java`）：

通过 Flink CDC 从 PostgreSQL 读取维度表变更数据，解析 Debezium JSON 格式并路由到不同的 Fluss 维度表。

**核心流程**：
1. 从 PostgreSQL CDC 读取 `public.dim_user`、`public.dim_singer`、`public.dim_song` 表的变更数据
2. 解析 Debezium JSON 格式的 CDC 数据（`op` 操作类型、`before/after` payload、`source` 表名）
3. 根据表名将数据路由到不同的侧输出流（`USER_TAG`、`SINGER_TAG`、`SONG_TAG`）
4. 将数据写入对应的 Fluss 维度表（`dim_user`、`dim_singer`、`dim_song`）

**CDC 数据格式示例**：
```json
{
  "offset": 3,
  "timestamp": 1771555059638,
  "change_type": "INSERT",
  "row": {
    "user_id": 10032,
    "user_name": "田雨",
    ...
    "gender": "女",
    "email": null,
    "birthday": "+27060-03-07T00:00:00",
    "channel": "PC",
    "province": "广东省",
    "register_ts": "+54731-02-08T13:06:40",
    "vip_end": null,
    "etl_load_date": "2026-02-20"
  }
}
```
...
**数据转换字段**：
- `dim_user`：user_id, user_name, phone, gender, email, birthday, channel, province, register_ts, vip_end, etl_load_date
- `dim_singer`：singer_id, singer_name, gender, fans, country, birthday, etl_load_date
- `dim_song`：song_id, song_name, singer_id, song_type, song_ts, song_duration, etl_load_date
...
主要配置位于 `Project_Common/src/main/java/common/Common.java`，包含以下关键配置项：

- **Kafka 配置**：`KafkaBootstrapServers`
- **StarRocks 配置**：`StarRocksJdbcUrl`、`StarRocksFeNodes`、`StarRocksUsername`
- **OSS 配置**：`OssAccessKeyId`、`OssAccessKeySecret`、`OssEndpoint`、`WarehousePath`、`CheckPointPath`
- **Fluss 配置**：`Fluss_BootstrapServers`
- **PostgreSQL 配置**：`Pg_host`、`Pg_port`、`Pg_DataBase`
